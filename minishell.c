...
void	ft_findcmd(t_data *data)
{
	...
	if (data->pid == 0)
	{
		...
		data->true_path = ft_get_access(data, data->splitl[0]);
		if (execve(data->true_path,  &data->splitl[0], data->newenv) == -1)
			printf("error execve\n");
	}
	else
...
{
	if (ft_strcmp(data->splitl[0], "echo") == 0)
		return ;
	if (ft_strcmp(data->splitl[0], "cd") == 0)
		ft_changedir(data, data->splitl[1]);
	else if (ft_strcmp(data->splitl[0], "pwd") == 0)
		ft_pwdorenv(data->newenv, "PWD");
	// else if (ft_strcmp(data->splitl[0], "export") == 0)
	// 	ft_echo(data, data->splitl[1]); // changer le path avec find path apres un export
	// else if (ft_strcmp(data->splitl[0], "unset") == 0)
	// 	ft_echo(data, data->splitl[1]); //
	else if (ft_strcmp(data->splitl[0], "env") == 0)
		ft_pwdorenv(data->newenv, "ENV");
	else if (ft_strcmp(data->splitl[0], "exit") == 0)
		ft_exit(data, data->splitl);
	else
		ft_findcmd(data);
}
...
int	main(int argc, char **argv, char **env)
{
	...
	while (1)
	{
		...
		// data->splitl = ft_split(data->line, ' ');
		// ft_whoitis(data);
	}
}
